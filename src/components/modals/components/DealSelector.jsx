...
export function DealSelector({
    ...
    useEffect(() => {
        if (!hasDeals || disabled || mode !== 'deal') {
            return;
        }

        const fallbackId = selectedDealId || normalizedDeals[0]?.id;
        if (fallbackId && fallbackId !== selectedDealId) {
            onDealChange?.(fallbackId);
        }
    }, [hasDeals, disabled, mode, normalizedDeals, onDealChange, selectedDealId]);
